...
class Groups:
    ...
    def save_data(self, name, exam_id):
        ...
        try:
            with self.connection.cursor() as cursor:
                sql = """
                INSERT INTO `grupy` (name, exam_id)
                VALUES (%s, %s)
                """
                cursor.execute(sql, (name, exam_id))
    ...
    def delete_selected_record(self, table_widget):
        ...
        try:
            with self.connection.cursor() as cursor:
                sql_query = "DELETE FROM grupy WHERE group_id = %s"
    ...
    def load_data_to_tablewidget(self, table_widget, query=None):
        try:
            with self.connection.cursor() as cursor:
                if query is None:
                    sql_query = "SELECT group_id, name, exam_id FROM grupy"
                else:
                    sql_query = query
        ...
        try:
            with self.connection.cursor() as cursor:
                ...
                for row in range(row_count):
                    ...
                    exam_item = table_widget.item(row, 2)

                    # Debug prints
                    print(f"Row {row}:")
                    print(f"ID: {id_item.text() if id_item else 'None'}")
                    print(f"Name: {name_item.text() if name_item else 'None'}")
                    print(f"Exam ID: {exam_item.text() if exam_item else 'None'}")

                    if all([id_item, name_item, exam_item]):
                        try:
                            ...
                            update_query = """
                                UPDATE `grupy`
                                SET name = %s, exam_id = %s
                                WHERE group_id = %s
                            """
                            cursor.execute(update_query, (name, exam_id, group_id))
    ...
    def load_combobox_data(self, comboBox):
        try:
            with self.connection.cursor() as cursor:
                ...
                comboBox.clear()
                for exam in exams:
                    comboBox.addItem(exam[1], exam[0])
