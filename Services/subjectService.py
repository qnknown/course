...
class Subjects:
    ...
    def delete_selected_record(self, table_widget):
        ...
        try:
            with self.connection.cursor() as cursor:
                sql_query = "DELETE FROM subjects WHERE subjectID = %s"
    ...
    def load_data_to_tablewidget(self, table_widget, query=None):
        try:
            with self.connection.cursor() as cursor:
                if query is None:
                    sql_query = "SELECT subjectID, name FROM subjects"
                else:
                    sql_query = query
        ...
        try:
            with self.connection.cursor() as cursor:
                ...
                for row in range(row_count):
                    ...
                    # Debug prints
                    print(f"Row {row}:")
                    print(f"subjectID: {id_item.text() if id_item else 'None'}")
